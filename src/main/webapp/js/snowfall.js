    ...
    function jSnow(){
        ...
        var defaults = {
            ...
            flake = function(_el, _size, _speed)
            {
            	...
            	this.x = (Math.round(Math.random()*10)%2==0)?widthOffset+10:elWidth - widthOffset-70;
                //this.y  = random(0, elHeight);
                this.y=0; //一定是从顶部开始出现 modified by danluo.
                this.size = _size;
                this.speed = _speed;
                this.step = 0;
                ...
                if(defaults.image){
					...
					flakeObj = new Image();
					//modified by danluo 可能就一个奖品
					if(defaults.image.length==1)
					{
						flakeObj.src = defaults.image[0];
					}	
					else
					{
                ...
                this.reset = function(){
                    this.y = 0;
                    //this.x = random(widthOffset, elWidth - widthOffset);              
                    this.x = (Math.round(Math.random()*10)%2==0)?widthOffset+10:elWidth - widthOffset-70; //控制x坐标为屏幕左侧或右侧
                    this.stepSize = random(1,15) / 100;
