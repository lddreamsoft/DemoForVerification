...
<link rel="stylesheet" href="/hlhh/css/winning.css" type="text/css" />
<style type="text/css">
...
<body>
	...
	<audio id="audio_Shake" preload="auto" src="/hlhh/audio/shake.mp3">
	</audio>

	<audio id="audio_Winning" preload="auto" src="/hlhh/audio/winning.mp3">
	</audio>

	<div class="container text-center">
		<div class="row">

			<div class="" style="background-color: #ffdc65; position: relative; background-image: url(/hlhh/images/luckyShake/bg_radiation.png); background-size: 100% auto; background-repeat: no-repeat;">
				<img src="/hlhh/images/luckyShake/1.png" class="img-responsive"/>
					
				<img onclick="showRules();" src="/hlhh/images/common/tips.png" style="position:absolute;z-index:2;width:5rem;height:5rem;right:1rem;top:9rem;" />
				...
				<div style="position: relative;">
					<img src="/hlhh/images/luckyShake/4.png" class="img-responsive"
						  style="" /> <img src="/hlhh/images/luckyShake/3.png"
						class="shake img-responsive"  
						style="position: absolute; width: 50%; left: 22%; top: 40%;" /> <img
						src="/hlhh/images/luckyShake/2.png" class="img-responsive"
				...
				<div style="text-align: center; margin-top: 10%; position: relative;">
					<img src="/hlhh/images/luckyShake/5.png" class="img-responsive"
...
<script src="/hlhh/js/layer/mobile/layer.js"></script>
<script src="/hlhh/js/snowfall.js"></script>
<script src="/hlhh/js/tools.js"></script>
<!--<script src="/hlhh/js/jweixin-1.0.0.js"></script>暂不提供分享功能-->
<script>
...
function showRules()
{
	...
	if(!clicked)
	{
		clicked=true;
		//获取活动规则
		$.get("/hlhh/activities/queryById/activity/1/0",function(result) {
	...
	$.ajaxPrefilter(function(options, originalOptions, jqXHR) {
		if (options.url.indexOf('/hlhh/wx/activities/luckyShake/draw') >= 0) {
	...
	function deviceMotionHandler(eventData) {
		...
		var curTime = new Date().getTime();
		{
			if ((curTime - last_update) > 450) { //多次移动事件中取两个点的事件间隔
				...
				$.ajax({ url : "/hlhh/wx/activities/luckyShake/draw/1/${openID}",
						type : "GET",
							...
							else if(result.chance==-1||result.chance==-2)
							{
								...
								layerIndex=layer.open({
								  ...
								  success: function(elem){
									  /*奖品只取第一张图片*/
									  $("#raysLogo").attr("style","background: url(/hlhh/upload/images/"+result.pictures.split(',')[0]+") 0 0 no-repeat;"+
									  							  "position: absolute; "+
								  ...
								  shade: 'background-color: rgba(0,0,0,.7)',
								  content: 
									  "<div id='raysDemoHolder'><em style='text-align:center;font-size:20px;color:white;position:absolute;width:20rem;margin-left:10rem;'>"+winningWords[Math.round(Math.random()*1000)%15]+"&nbsp;!</em>"+
	...
	{
		$.get("/hlhh/wx/activities/luckyShake/winningRecord/1/0/10",
				function(result) {
	...
	{
		//result.limits==0?
		$.get("/hlhh/wx/activities/luckyShake/winningRecord/1/${openID}",function(result)
				{
					if(result!='')
					{
						$("#div_MyPrizes").empty();
	...
	$(function() {
		...
		*/
		...
		//判断活动是否开始，活动未开始提示用户
		$.get("/hlhh/activities/queryById/activity/1/1",function(result){
		...
		$.get("/hlhh/wx/member/queryMemberPoints/${openID}",function(result)
		{
		  var type=0;
			  ...
			  //if(result.isSubscribed==1)不管是否关注，显示会员积分
			  {
				  ...
				  else
				  {
					  ...
					  else
					  {
						  ...
						  $.get("/hlhh/wx/activities/luckyShake/getDrawTimesOfToday/1/${openID}",function(result){
		  ...
		  else
		  {
			 ...
			 //将openID插入微信会员表
			 $.get("/hlhh/wx/member/insertWXMember/${openID}",function(result)
			 {
				 //无处理
			 });
		...
		//读取奖品图片 性能优化点
		$.get("/hlhh/activities/selectAvailablePrizes/1",function(result){
	      if(result!='')
		  {
	    	var pictures=[];
	    	
	    	$(result).each(function(index, value) {
	    	 pictures[index]="/hlhh/upload/images/"+value.pictures.split(',')[0];
	      	});
